...
//!
//! Reads an optional `flyway-linter.yml` (or a path supplied via `--config`).
...
impl Config {
    ...
    pub fn from_file(file: ConfigFile) -> Result<Self, Vec<String>> {
        ...
        for rule in file.rules.keys() {
            if !known.iter().any(|&k| k == rule.as_str()) {
                errors.push(format!(
...
pub const DEFAULT_CONFIG_NAME: &str = "flyway-linter.yml";

/// Load and validate a config file.
///
/// - `explicit_path`: a path from `--config`, must exist.
/// - If `None`, looks for `flyway-linter.yml` in the current directory.
...
/// Canonical list of all rule identifiers.
/// Used for config validation — any key not in this list is an error.
...
mod tests {
    ...
    fn danger_level_maps_to_danger_severity() {
        assert_eq!(Config::to_severity(&RuleLevel::Danger), Some(Severity::Danger));
    }

    #[test]
    fn warn_level_maps_to_warning_severity() {
        assert_eq!(Config::to_severity(&RuleLevel::Warn), Some(Severity::Warning));
    }
