...
impl std::str::FromStr for OutputFormat {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            other  => Err(format!("unknown format `{other}` — valid values: text, gha")),
        }
    }
}
...
fn print_text_summary(results: &[FileResult]) {
    ...
    println!(
        "Checked {}  ·  {} file(s) with issues",
        format!("{total} migration(s)"),
...
fn print_gha_summary(results: &[FileResult]) {
    ...
    if issues == 0 {
        println!("::notice::flyway-linter: all {} migration(s) passed", results.len());
    } else {
        println!(
            "::notice::flyway-linter: {} danger, {} warning across {} file(s)",
...
mod tests {
    ...
    use crate::finding::{Finding, FileResult, Severity};
