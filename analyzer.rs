...
//!
//!   flyway-linter migrations/                          # directory
//!   flyway-linter migrations/V1__init.sql             # single file
//!   flyway-linter migrations/V1*.sql migrations/V2*.sql  # multiple patterns
//!   flyway-linter 'migrations/**/*.sql'               # glob (quoted to avoid shell expansion)
            ...
            for entry in paths {
                ...
                match entry {
                    Ok(path) => {
                        ...
                        } else if path.extension().map(|e| e.eq_ignore_ascii_case("sql")).unwrap_or(false) {
                            files.push(path);
                        }
...
pub fn analyze_file(path: &Path, config: &Config) -> FileResult {
    ...
    match Parser::parse_sql(&dialect, &sql) {
        Ok(stmts) => {
            let findings = stmts.iter().flat_map(|s| check_statement(s, path, config)).collect();
...
mod tests {
    ...
    fn resolve_mixed_file_dir_and_glob() {
        ...
        assert!(files.iter().any(|f| f.file_name().unwrap() == "V2__in_subdir.sql"));
        // No duplicates
